...
function render(id){
  ...
  const basic=color=>`<table>${header()}<tbody>${S.map((s,r)=>`<tr><th>${s}</th>${V[r].map((v,c)=>`<td${color?` style="background:${heat(v,V[0][c])}" title="${(v/V[0][c]*100).toFixed(1)}% от максимума дня"`:""}>${v}</td>`).join("")}</tr>`).join("")}</tbody></table>`;
  const delta=()=>{let rows=[];S.forEach((s,r)=>{rows.push(`<tr class="event-row"><th>${s}</th>${V[r].map(v=>`<td>${v}</td>`).join("")}</tr>`);if(r<S.length-1)rows.push(`<tr class="delta-row"><th>Потеря к следующему шагу</th>${V[r].map((v,c)=>`<td>${V[r+1][c]-v}</td>`).join("")}</tr>`)});return `<table>${header()}<tbody>${rows.join("")}</tbody></table>`};
  const percent=compare=>{let rows=[];S.forEach((s,r)=>{const am=avg(V[r].slice(1));rows.push(`<tr class="event-row"><th>${s}</th>${V[r].map((v,c)=>`<td class="${compare?cell(v,am,c):""}">${v}</td>`).join("")}<td class="average-cell">${am.toFixed(1)}</td></tr>`);if(r<S.length-1){const la=avg(L[r].slice(1).filter(v=>v!==null)),cs=V[r].map((v,c)=>{if(v===0)return"<td>—</td>";const z=L[r][c],h=120*(1-(z-lo)/(hi-lo));return `<td class="${compare?cell(z,la,c):""}" style="background:hsl(${h} 76% 85%)">${z.toFixed(1)}%</td>`});const h=120*(1-(la-lo)/(hi-lo));cs.push(`<td class="average-cell" style="background:hsl(${h} 76% 85%)">${la.toFixed(1)}%</td>`);rows.push(`<tr class="delta-row"><th>Потеря к следующему шагу, %</th>${cs.join("")}</tr>`)}});let foot="";if(compare)foot=`<tfoot><tr class="comparison-summary"><th>Синие / красные ячейки</th>${dates.map((_,c)=>{if(c===0)return"<td>—</td>";let b=0,r=0;S.forEach((_,i)=>{const a=avg(V[i].slice(1));if(V[i][c]>a)b++;if(V[i][c]<a)r++;if(i<S.length-1&&L[i][c]!==null){const q=avg(L[i].slice(1).filter(v=>v!==null));if(L[i][c]>q)b++;if(L[i][c]<q)r++}});return `<td><span class="blue-count">${b}</span> / <span class="red-count">${r}</span></td>`}).join("")}<td>—</td></tr></tfoot>`;return `<table class="${compare?"comparison-table":"percent-table"}">${header(true)}<tbody>${rows.join("")}</tbody>${foot}</table>`};
